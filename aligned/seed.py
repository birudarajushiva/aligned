"""The simulated portfolio Traction watches.
...
def generate_accounts(n: int = 12) -> List[Account]:
    ...
    if n > len(PROFILES):
        raise ValueError(
            ...
            "Add another entry to traction.seed.PROFILES rather than generating "
...
def verify_portfolio(accounts: List[Account]) -> Dict[str, int]:
    ...
    if counts != TARGET_MIX:
        raise AssertionError(
            ...
            "traction/seed.py, not the rules in traction/scoring.py."
            % (counts, TARGET_MIX)
        )
    ...
    if not hero:
        raise AssertionError(
            ...
            "needs one — see acc_001 in traction/seed.py."
        )
